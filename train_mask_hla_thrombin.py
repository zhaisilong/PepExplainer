...
def run_a_model(seed: Optional[int] = None):
    ...
    origin_data_dir = Path(f"./data/{task}/origin_data")
    graph_data_dir = Path(f"./data/{task}/graph_data")
    ...
    stopper = EarlyStopping(
        ...
        seed=seed,
        mode=args["mode"],
        # pretrained_model="../model/enrich_reg_aa_0_early_stop.pth"
    )
    ...
    for epoch in range(args["num_epochs"]):
        ...
        print(f"train stage: train loss {total_loss}, val score: {val_score[0]}")
