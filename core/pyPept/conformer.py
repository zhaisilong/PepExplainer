...
# pyPept modules
from core.pyPept.sequence import Sequence
from core.pyPept.molecule import Molecule
from core.pyPept.sequence import SequenceConstants
from core.pyPept.sequence import get_monomer_info
...
class Conformer:
    ...
    def get_peptide(
        ...
        try:
            ...
            warnings.warn(
                f"No main peptide was detected for peptide \
                          with BILN: {biln}"
            )
            sys.exit(1)
                ...
                if type_mon != "cap":
                    ...
                    if nat_analog != "X":
                        ...
                        if val >= 0.5:
                            ...
                            warnings.warn(
                                f"Because no natural amino acids were \
    ...
    def generate_conformer(
        romol, ss_value, generate_pdb=False, output_name="structure"
    ):
        """
        ...
        invalid = [
            v for v in ss_value if v not in ConformerConstants.expected_ss_symbols
        ]
        if len(invalid) > 0:
            raise RuntimeError(
...
class SecStructPredictor:
    ...
    def align_position_matrix(
        ...
        if (
            path != ConformerConstants.def_path
            or matrix_lib != ConformerConstants.def_matrix_filename
        ):
            matrix = SecStructPredictor.get_matrix(os.path.join(path, matrix_lib))
        else:
            try:
                stream = pkg_resources.resource_stream(
                    __name__, os.path.join(path, matrix_lib)
                )
                matrix = {}
                ...
                matrix = SecStructPredictor.get_matrix(
                    os.path.join(
    ...
    def similarity_pair(
        ...
        score1_2 = SecStructPredictor.align_position_matrix(
            peptide1, peptide2, path=path, matrix_lib=matrix_lib
        )
        score1_1 = SecStructPredictor.align_position_matrix(
            peptide1, peptide1, path=path, matrix_lib=matrix_lib
        )
        score2_2 = SecStructPredictor.align_position_matrix(
            peptide2, peptide2, path=path, matrix_lib=matrix_lib
        )
    ...
    def predict_active_ss(
        ...
        if (
            path != ConformerConstants.def_path
            or ss_lib != ConformerConstants.def_ss_filename
        ):
            total_ss = [
                x.strip() for x in open(os.path.join(path, ss_lib), encoding="utf8")
            ]
        else:
            try:
                stream = pkg_resources.resource_stream(
                    ...
                    for i in range(0, len(seq) - len(sequence) + 1):
                        ...
                        if score >= int(len(sequence) * 2 / 5):
                            similarity = SecStructPredictor.similarity_pair(
                                sequence, fragment, path=path, matrix_lib=matrix_lib
                            )
